...
function authenticate($user, $password) {
    ...
    if ($bind) {
        ...
        foreach($info[0]['memberof'] as $grps) {
            ...
            if(strpos($grps, $ldap_manager_group)) { $access = 3; break;} 
            // is tech user
            else if(strpos($grps, $ldap_tek_group)){ $access = 2; break;}
            // is staff user
            else if(strpos($grps, $ldap_staff_group)){ $access = 1;}
        }
